...
abstract class Model implements \JsonSerializable, \ArrayAccess
{
    ...
    /*
     * 获取结果集中某个字段的值
     * 适用于调用select之后，返回的数组中存放的是多个模型（Model）对象,
     * 且每项的对象的父类是think\Model的情况
     * @access public
     * @param array   $obj_arr 结果集
     * @param string   $field 字段名称
     * @return array
     * */
    public static function getResultByFild($obj_arr,$field='*',$key='')
    {
        if(!is_array($obj_arr) || empty($obj_arr)) return array();
        if(!is_string($field) || empty($field)) $field="*";

        if($key!=''){
            $a0=$obj_arr[0];
            if(!array_key_exists($key,$a0->data)) $key='';
        }


        $return_global_arr = array();
        if($field!='*'){
            $return_arr = array_map(function ($value) use ($field,$key,&$return_global_arr)  {
                $v='';
                if(array_key_exists($field,$value->data)){
                    $v = ('think\\Model'==get_parent_class($value)) ? $value[$field] : '';
                }
                if($key!=''){
                    $return_global_arr[$value[$key]]=$v;
                }else{
                    $return_global_arr[]=$v;
                }
            }, $obj_arr);
            $return_global_arr = array_filter($return_global_arr);
            return $return_global_arr;
        }else{
            array_map(function ($value) use($key,&$return_global_arr){
                $v=('think\\Model'==get_parent_class($value)) ? $value->toArray() : $value;
                if($key!=''){
                    $return_global_arr[$value[$key]]=$v;
                }else{
                    $return_global_arr[]=$v;
     ...
     * @access public
     * @param array   $obj_arr 结果集
     * @param string   $function 要被调用的回调函数名
     * @return array
     * */
    public static function callByResult($obj_arr,$function,$params=array())
    {
        if(!is_array($obj_arr) || empty($obj_arr)) return;
        if (is_string($function) && !empty($function))
        {
            array_map(function($value) use ($function,$params){
                    if('think\\Model'==get_parent_class($value)){
                        $call_back=[$value,$function];
                        if (method_exists($value, $function)) {
                            $result=call_user_func_array($call_back, $params);
