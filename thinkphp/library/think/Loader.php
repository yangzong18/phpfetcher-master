...
class Loader
{
	 ...
	 * filter过滤
	 * **/
	public static function gofilter($data)
	{
		if(is_array($data) && count($data)>0){
			foreach($data as $key=>$v){
				if(is_string($v)){
					$data[$key]= self::fliterHtmlSpecialChars($v);
				}else{
					$data[$key]=self::gofilter($v);
				}
			}
		}
		return $data;
	 ...
	 * @return unknown
	 */
	private static function fliterHtmlSpecialChars($string)
	{
		//数据库非法字符串
		$db_arr =array('/and /Ui','/sleep/Ui','/select /Ui','/update /Ui','/delete from/Ui','/insert into/Ui','/drop /Ui','/information_schema /Ui','/alter /Ui','/where /Ui','/case when /Ui');
		$replacements_array=array('','','','','','','','','','');
		$string=preg_replace ( $db_arr ,  $replacements_array ,  $string );

		//跨站脚本
		$Xs_arr =array('/script/Ui','/<script/Ui','/&lt;script/Ui','/alert\(/Ui','/document\./Ui','/window\./Ui','/cookie\)/Ui');
		$replacements_array=array('','','','','','','');
		$string=preg_replace ( $Xs_arr ,  $replacements_array ,  $string );
...
//李武添加
$_POST=Loader::gofilter($_POST);
$_GET=Loader::gofilter($_GET);
