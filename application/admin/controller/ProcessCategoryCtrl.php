...
class ProcessCategoryCtrl extends Auth
{
    protected $model;
    protected $field = '*';
    ...
    public function __construct() {
        parent::__construct();
        $this->model = new ProcessCategory();
        $this->model_set = new ProcessSetting();
    }
    ...
    public function addPost() {
        ...
        $param['sort'] = $this->request->param('sort', 'intval', 0);

        if ( trim( $param['name'] ) == '' )  $this->error('分类名称不能为空');
    ...
    public function delete() {
        $cate_id = $this->request->param('id', 0, 'intval');
        if (!is_numeric($cate_id)){
            $this->error('删除失败');
        }
        if ( $this->model->save( array('is_delete' => 1),array('cate_id' => $cate_id)) ) {
             $this->success('删除成功');
        } else {
            $this->error('删除失败');
        }
