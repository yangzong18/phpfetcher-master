...
class AopClient {
	...
	protected function getSignContent($params,$log=0)
	{
		ksort($params);
		$stringToBeSigned = "";
		$i = 0;
	...
	*/
	public function pageExecute($request,$httpmethod = "POST") {
		...
		//2.签名
		$sign = $this->sign($preSignStr, $this->signType);

		//3.对代签名字符串的value进行urlencode
		$preSignStr=$this->getSignContent($totalParams,1);
		...
		if ("GET" == $httpmethod) {
			...
			//拼接GET请求串
			$requestUrl = $preSignStr."&sign=".urlencode($totalParams["sign"]);
			return $requestUrl;
		} else {
			//拼接表单字符串
