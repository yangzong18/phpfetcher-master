...
class SDKConfig {
	...
	public static function getSDKConfig($payment_info=array()){
		if (SDKConfig::$_config == null ) SDKConfig::$_config = new SDKConfig($payment_info);
	...
	function __construct($payment_info=array()){
		...
		$configFilePath = PAYPATH . "/sdk/acp_sdk.ini";
        ...
        $this->signCertPath = array_key_exists("acpsdk.signCert.path", $sdk_array)?PAYPATH.'/certs/'.$sdk_array["acpsdk.signCert.path"]: null;
        $this->signCertPwd = !empty($payment_info)?$payment_info['payment_config']['chinabank_key']: null;
		$this->validateCertDir = array_key_exists("acpsdk.validateCert.dir", $sdk_array)? $sdk_array["acpsdk.validateCert.dir"]: null;
		$this->encryptCertPath = array_key_exists("acpsdk.encryptCert.path", $sdk_array)? PAYPATH.'/certs/'.$sdk_array["acpsdk.encryptCert.path"]: null;
		$this->rootCertPath = array_key_exists("acpsdk.rootCert.path", $sdk_array)? PAYPATH.'/certs/'.$sdk_array["acpsdk.rootCert.path"]: null;
		$this->middleCertPath =  array_key_exists("acpsdk.middleCert.path", $sdk_array)?PAYPATH.'/certs/'.$sdk_array["acpsdk.middleCert.path"]: null;
		...
		$this->logFilePath =  array_key_exists("acpsdk.log.file.path", $sdk_array)?PAYPATH.'/logs/'.$sdk_array["acpsdk.log.file.path"]: null;
